...
async def process_pending_file(file_path):
    ...
    output_path = f"{OUTPUT_DIRECTORY}/{file_name}.json"

    try:
        process = await asyncio.create_subprocess_exec("python3", explainer, file_path, output_path)
        await process.communicate()
        upload = session.query(Upload).filter_by(filename=filename).first()
        if os.path.exists(output_path) and upload:
            # Update the database with the new status and filename
    ...
    try:
        while True:
            ...
            for upload in pending_uploads:
                file_path = os.path.join(UPLOAD_DIRECTORY, upload.filename)
                await process_pending_file(file_path)
